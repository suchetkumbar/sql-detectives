...
export function RapidFire({ intro, questions, timePerQuestion, onComplete }: Props) {
  ...
  function handlePick(i: number) {
    ...
    setTimeout(() => {
      if (idx + 1 >= questions.length) {
        ...
        onComplete({ passed: finalCorrect >= Math.ceil(questions.length * 0.6), correct: finalCorrect, total: questions.length });
      } else {
        setIdx(idx + 1); setPicked(null); setTime(timePerQuestion);
      }
    }, 1100);
  }
  ...
  if (!started) {
    return (
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="surface rounded-xl p-8 text-center max-w-2xl mx-auto">
        <span className="chip chip-dot text-destructive mb-4"><span>Rapid fire</span></span>
        <h2 className="text-3xl md:text-4xl mt-2 text-balance">{intro}</h2>
        <p className="mt-4 text-sm text-muted-foreground mono">{questions.length} questions · {timePerQuestion}s each · 60% to pass</p>
        <button
  ...
  return (
    <div className="surface rounded-xl p-6 md:p-8 max-w-3xl mx-auto">
      ...
      <div className="h-1 bg-secondary mt-2 rounded overflow-hidden">
        <div className={`h-full transition-[width] duration-1000 linear ${time <= 3 ? "bg-destructive" : "bg-primary"}`} style={{ width: `${pct}%` }} />
      </div>
              ...
              return (
                <button
                  ...
                  className={[
                    ...
                    show && isPicked ? "border-destructive bg-destructive/10" :
                    "border-border bg-secondary/30 hover:border-primary/60",
                  ].join(" ")}
                >
                  <span className="mono text-xs text-muted-foreground mr-2">{String.fromCharCode(65 + i)}</span>
                  {opt}
                </button>
              );
            })}
